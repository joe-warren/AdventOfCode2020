...
rotateCompass :: Int -> Compass -> Compass
rotateCompass r c = toEnum $ (r + fromEnum c) `mod` 4
...
executeInstruction (Direction d v) (BoatState p c) = BoatState (moveDirection d v p) c
executeInstruction (Forward v) (BoatState p c) = BoatState (moveDirection c v p) c
executeInstruction (Rotation r) (BoatState p c)  = BoatState p (rotateCompass r c)
...
main = do 
    ...
    case parseFile content of
        ...
        (Right records) -> do 
            ...
            let res = foldl (flip executeInstruction) (BoatState (0, 0) E) records
            let (x, y) = boatPos res
            let r = abs x + abs y 
            print res
            print r
